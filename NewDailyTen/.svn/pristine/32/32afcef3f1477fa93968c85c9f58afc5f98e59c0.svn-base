...
+ (NSString *)getSinaNickNameDelegate:(id)delegate{
    if (![[NSUserDefaults standardUserDefaults] objectForKey:@"sina_nickName"]) {
        if ([[SinaHelper getHelper] sinaIsAuthValid]) {
            [helper getUserInfo];
        }
    }
   
   return [[NSUserDefaults standardUserDefaults] objectForKey:@"sina_nickName"];
}
...
- (BOOL)sinaIsAuthorizeExpired{
    return [[self sinaweibo] isAuthorizeExpired];
}
...
//    return appDelegate.sinaweibo;
    if (!mSinaweibo) {
        mSinaweibo = [[SinaWeibo alloc]initWithAppKey:sinaAppKey appSecret:sinaAppSecret appRedirectURI:sinaAppRedirectURI andDelegate:helper];
        [self refreshMessage];
    }
    return mSinaweibo;
}
...
- (void)refreshMessage{
    ...
    mSinaweibo.accessToken = [sinaweiboInfo objectForKey:@"AccessTokenKey"];
    mSinaweibo.expirationDate = [sinaweiboInfo objectForKey:@"ExpirationDateKey"];
    mSinaweibo.userID = [sinaweiboInfo objectForKey:@"UserIDKey"];
}
...
- (void)login//登陆
{
   ...
   // NSLog(@"%@", [keyWindow subviews]);
    ...
    [[NSUserDefaults standardUserDefaults] removeObjectForKey:@"sina_nickName"];
     _userInfo = nil;
     _statuses = nil;
    SinaWeibo *sinaweibo = [self sinaweibo];
...
{
    NSLog(@"sinaweiboDidLogIn userID = %@ accesstoken = %@ expirationDate = %@ name = %@", sinaweibo.userID, sinaweibo.accessToken, sinaweibo.expirationDate, sinaweibo.name);
    [self storeAuthData];
...
- (void)sinaweiboLogInDidCancel:(SinaWeibo *)sinaweibo
{
    ...
    if ([_delegate respondsToSelector: @selector(sinaDidLoginCancel)]) {
        [_delegate performSelector: @selector(sinaDidLoginCancel)];
    }
...
- (void)request:(SinaWeiboRequest *)request didFailWithError:(NSError *)error
{
    ...
    else if ([request.url hasSuffix:@"2/comments/create.json"]){
        // 评论一篇帖子成功
        if ([_delegate respondsToSelector: @selector(sinaCommentFail:)]) {
            [_delegate performSelector: @selector(sinaCommentFail:) withObject: _userInfo];
        }
    }
    
    if([_delegate respondsToSelector: @selector(removeMovieAndPhoto)])
    {
        [_delegate performSelector: @selector(removeMovieAndPhoto) withObject: nil];
    }
    //    [self resetButtons];
}
...
- (void)request:(SinaWeiboRequest *)request didFinishLoadingWithResult:(id)result
{
    // 当一个 delegate 需要处理多个请求回调时,可以通过 url 来判断当前的 request
     _userInfo = result ;
    if ([request.url hasSuffix:@"users/show.json"]) {
        [[NSUserDefaults standardUserDefaults] setObject:[result objectForKey:@"screen_name"] forKey:@"sina_nickName"];
    ...
    // 获取我关注人的list
        ...
        // 评论一篇帖子成功
        if ([_delegate respondsToSelector: @selector(sinaCommentSuccess:)]) {
            [_delegate performSelector: @selector(sinaCommentSuccess:) withObject: _userInfo];
        }
    }
    else if ([request.url hasSuffix:@"short_url/shorten.json"]){
        // 长连接转短链接成功
...
// 转换成flash 连接
- (NSString *)joinlongUrl:(NSString *)movieUrl{
    return movieUrl;
}

// 发表图片到新浪weibo
- (void)uploadPicture:(NSData *)picData status:(NSString *)status target:(id)target movieUrl:(NSString *)movieUrl
{
    [self longUrlTOShortUrl:[self joinlongUrl:movieUrl]];
    mMovieUrl = movieUrl;
    isShare = YES;
...
- (void)commitCommentToSinaFieldID:(NSString *)sinaID comment:(NSString *)comment delegate:(id)delegate{
    SinaWeibo *sinaweibo = [self sinaweibo];
    self.delegate = delegate;
    [sinaweibo requestWithURL:@"comments/create.json" params:[NSMutableDictionary dictionaryWithObjectsAndKeys:sinaID ,@"id",comment ,@"comment" , nil] httpMethod:@"POST" delegate:self];
 }
